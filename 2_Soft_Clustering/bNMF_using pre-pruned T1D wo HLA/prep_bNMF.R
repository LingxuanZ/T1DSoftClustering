...
fetch_summary_stats <- function(df_variants, gwas_ss_file, trait_ss_files, trait_ss_size=NULL, pval_cutoff=1) {
  ...
  read_single_trait <- function(trait, variant_df) {
    ...
    # print(head(df))
    df <- df %>%
      rename(any_of(valid_rename_cols)) %>%
      filter(!duplicated(hm_variant_id)) %>%
      separate(hm_variant_id, into=c("CHR", "POS", "REF", "ALT"), sep="_") %>%
      mutate(SNP=paste(CHR, POS, sep=":")) %>%
      mutate(Effect_Allele_PH = if("Effect_Allele_PH" %in% names(.)) Effect_Allele_PH else ALT) %>%
      select(any_of(c('SNP', 'Effect_Allele_PH', 'REF', 'ALT', 'N_PH', 'hm_beta', 'standard_error', 'p_value', 'z'))) %>%
    ...
    if (!"z" %in% names(df)){
      df <- df %>%
        mutate(standard_error = ifelse(standard_error==0, min(standard_error[standard_error>0]), standard_error)) %>%
        mutate(z = hm_beta / standard_error)  # First, calculate z-score magnitude
    }
    df <- df %>%
  ...
  if (class(gwas_ss_file)=="character"){
    ...
    gwas_ss <- fread(gwas_ss_file, data.table=F, stringsAsFactors=F) %>%
      filter(hm_variant_id %in% variant_vec)
  } else {
